"""
core/ingestion.py
Resilient CTPL ERP Excel loader with multi-format date parsing
...
def _find_header_row(file) -> int:
    if hasattr(file, "seek"):
        file.seek(0)
    preview = pd.read_excel(file, header=None, nrows=15, dtype=str)
    if hasattr(file, "seek"):
        file.seek(0)
        
    for idx, row in preview.iterrows():
        row_vals = [str(v).strip() for v in row.values if pd.notna(v)]
        matches = sum(1 for c in REQUIRED_COLS if c in row_vals)
        if matches >= 3:
            return idx
    return 1  # Fallback
...
def load_erp_excel(file) -> pd.DataFrame:
    """
    Load and validate a raw CTPL ERP Excel export.
    Dynamically finds the header row (usually 0 or 1).
    """
    header_idx = _find_header_row(file)
    ...
    if missing_cols:
        st.error(
            ...
            "Cannot calculate accurate operational metrics without these columns. Please ensure you are uploading the correct CTPL ERP export."
        )
        st.stop()
    ...
    # Numeric coercion
    for col in ["OBQTY", "QC_OUT", "T", "LAY", "II", "REJ_QTY", "SFO_SHOT_QTY"]:
