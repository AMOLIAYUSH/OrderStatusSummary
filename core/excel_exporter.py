...
def export_to_excel(
    ...
    for group in TIER1_GROUPS:
        ...
        indices = sorted(column_index_from_string(col_map[c]) for c in present_cols)
        first_ltr = get_column_letter(indices[0])
        last_ltr  = get_column_letter(indices[-1])
    ...
    for row_offset, (_, data_row) in enumerate(df.iterrows()):
        ...
        for col_name, col_letter in col_map.items():
            ...
            elif col_name == "Plan Order Status":
                val = str(data_row.get(col_name, "")) if pd.notna(data_row.get(col_name, "")) else ""
                cell.value = val
                if val == "Ready":
                    _style_cell(cell, fill=PatternFill("solid", fgColor="86EFAC"), bold=True, align="center")
                elif "pending" in val:
                    _style_cell(cell, fill=PatternFill("solid", fgColor="FEF08A"), bold=True, align="center")
                else:
                    _style_cell(cell, fill=row_fill, align="left")
